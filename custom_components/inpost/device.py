...
ATTRIBUTION = "Data provided by InPost"


def build_device_info(entry: ConfigEntry) -> DeviceInfo:
    """Return the DeviceInfo shared by every entity of this account.

    The phone number is part of the name, so two configured accounts do not
    produce two indistinguishable devices. Entities inherit it via
    ``has_entity_name``, yielding names like
    "InPost (600123456) Incoming parcels".
    """
    return DeviceInfo(
        identifiers={(DOMAIN, entry.entry_id)},
        name=f"InPost ({entry.title})",
        manufacturer="InPost",
